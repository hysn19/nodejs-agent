...
function Keystore(accountName, config = {}) {
  ...
  userStorage.query(
    ...
    [accountName, 'kpath'],
    ([path, pubkey], wif) => {
      const storageKey = userStorage.createKey(
        accountName,
        'kpath',
        path,
        pubkey
  ...
  */
  function deriveKeys({ parent, saveKeyMatches = [], accountPermissions }) {
    keepAlive()
    ...
    assert(
      /master|wif|privateKey/.test(keyType),
    ...
    assert(
      typeof accountPermissions === 'object' || accountPermissions == null,
    ...
    if (!accountPermissions) {
      const permissions = userStorage.get(localStorage, [
        accountName,
        'permissions'
      ])
    ...
    if (keyType === 'master') {
      ...
      parentKeys.active = Keypair(
        parentKeys.owner.privateKey.getChildKey('active')
      )
      if (!accountPermissions) {
        accountPermissions = [
  ...
  */
  function addKey(path, key, toDisk = false) {
    ...
    assert(
      /^wif|pubkey|privateKey$/.test(keyType),
    ...
    const wif =
      keyType === 'wif'
        ? key
        : keyType === 'privateKey'
        ? ecc.PrivateKey(key).toWif()
        : null

    const pubkey =
      keyType === 'pubkey'
        ? ecc.PublicKey(key).toString()
        : keyType === 'privateKey'
    ...
    if (toDisk) {
      const saved = userStorage.save(localStorage, storageKey, wif, {
        clobber: false
      })
      dirty = dirty || saved
    }
  ...
  */
  function getPrivateKeys(keyPathMatcher = '**', pubkeys) {
    ...
    getKeys(keyPathMatcher)
      .filter(key => pubkeys.has(key.pubkey))
      .forEach(key => {
        if (key.wif == null) {
  ...
  */
  function getKeys(keyPathMatcher = '**') {
    ...
    if (deriveKeys.length) {
      for (const derivedKey of deriveKeys) {
        // {path, privateKey}
  ...
  */
  function removeKeys(paths, keepPublicKeys = config.keepPublicKeys) {
    ...
    assert(
      paths instanceof Array || paths instanceof Set,
  ...
  */
  function signSharedSecret(otherPubkey, keyPathMatcher = '**') {
    assert(
      /pubkey|PublicKey/.test(validate.keyType(otherPubkey)),
      'otherPubkey'
    )
