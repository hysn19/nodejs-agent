...
describe("Keystore", () => {
  ...
  it("initialize from disk", async function() {
    ...
    console.dir(wif); // log
    console.dir(pubkey); // log

    keystore.addKey("active/mypermission", wif, true /*disk*/);

    keystore = Keystore("myaccount");
    console.dir(keystore.getKeyPaths()); // log
  ...
  it("save and get keys", async function() {
    ...
    assert.equal(
      keystore.getPublicKey("owner/cold"),
      cold.toPublic().toString()
    );
  ...
  it("removeKeys", async function() {
    ...
    assert.deepEqual(keystore.addKey("active", wif), {
      wif,
      pubkey,
      dirty: true
    });
  ...
  it("signSharedSecret", async function() {
    ...
    const clientProof = await clientKeystore.signSharedSecret(
      oneTimeServerPublic
    );
    ...
    const sharedSecret = oneTimeServerPrivate.getSharedSecret(
      clientProof.oneTimePublic
    );
